...
## Output
- If `savefits` is True, the masks will be saved as a FITS file. If you use a mask in CASA, please convert it to an .image format using CASA task `importfits` before applying it. If the number of axes in the FITS data is four, they are ordered as [RA, DEC, STOKES, FREQ].
